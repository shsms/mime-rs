...
mod tests {
    ...
    fn motions_stay_linear_on_a_large_blank_line_free_buffer() {
        ...
        let prog = r#"
            (report "fp" (forward-paragraph))
            (goto-char (point-max))
            (report "bp" (backward-paragraph))
            (report "sb" (skip-chars-backward "^Z"))
            (goto-char (point-min))
        ...
        // `skip-chars-*` return the distance moved, not the position.
        assert_eq!(report(&r, "sb"), "0", "backward-paragraph left point at 1");
        assert_eq!(report(&r, "sf"), (point_max - 1).to_string());
    ...
    #[test]
    fn long_skips_agree_on_both_stores_and_stay_linear() {
...
"
        ...
        let prog = r#"
            ...
            (goto-char (point-min))
            (report "w" (forward-word 3))
            (report "l" (skip-chars-forward "^
"))
        "#;
        let started = std::time::Instant::now();
        let (a, b) = on_both_stores("long-skips", &text, prog);
        let elapsed = started.elapsed();
        assert_eq!(a.reports, b.reports);
        assert_eq!(a.point, b.point);
        // No `Z` anywhere: the forward skip runs from 1 to point-max.
        assert_eq!(report(&a, "sf"), (point_max - 1).to_string());
        assert!(
            elapsed < std::time::Duration::from_secs(10),
            "the skips took {elapsed:?} over {point_max} chars on both stores; \
