...
| Group | Primitives |
|---|---|
| Motion | `point` `point-min` `point-max` `goto-char` `goto-line` `forward-char` `backward-char` `forward-line` `forward-word` `backward-word` `forward-paragraph` `beginning-of-buffer` `end-of-buffer` `beginning-of-line` `end-of-line` `back-to-indentation` `line-beginning-position` `line-end-position` `line-number-at-pos` `current-column` `current-indentation` |
