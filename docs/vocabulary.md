...
| Filling | `fill-paragraph` (variables `fill-column` = 80, `sentence-end-double-space` = t; the unit at point is a run of line comments, a block comment, a Python triple-quoted string or a Markdown paragraph — the tree-sitter parse says which, code is never reflowed, and a file type without a grammar is refused; comment markers, list hanging indents, fences and tables survive) |
