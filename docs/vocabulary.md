...
| Merge conflicts | `conflict-count` `conflict-hunks` `conflict-goto` `conflict-context` `conflict-text` `conflict-diff` `conflict-keep` `conflict-keep-all` `conflict-replace` `conflict-resolve-trivial` (git/diff3 markers; smerge-flavored) |
...
| Observability | `report` `message` `window` `occur` `buffer-file-name` `buffer-stale-p` `buffer-file-coding-system` `set-buffer-file-coding-system` |
