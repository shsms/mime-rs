...
| Structural (M7) | `treesit-language` `treesit-set-language` `treesit-root-type` `treesit-has-error` `treesit-beginning-of-defun` `treesit-end-of-defun` `treesit-defun-name` `treesit-narrow-to-defun` `treesit-list-defuns` `treesit-goto-defun` (tree-sitter; Markdown, Rust, Python) |
